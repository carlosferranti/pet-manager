...
public class DoisNiveisCacheTest : BaseCacheTest
{
    [Fact]
    public async Task DeveBuscarItemPrimeiroNivelCacheCacheEEncontrarAsync()
    {
        await ExecutarComSemaforoAsync(async () =>
        {
            var cache = CriarDoisNiveisCache<string>(BuscarItemAsync, true);
    ...
    [Fact]
    public async Task DeveBuscarItemSegundoNivelCacheEEncontrarAsync()
    {
        await ExecutarComSemaforoAsync(async () =>
        {
            var cache = CriarDoisNiveisCache<string>(BuscarItemAsync, true, 1);
            ...
            item.Should().Be(_existente);
            cache.ObterNumeroItensEncontradosCache(_existente, 1).Should().BeEmpty();
            cache.ObterNumeroItensEncontradosCache(_existente, 2).Should().NotBeEmpty();

            cache.ObterNumeroBuscas(_existente).Should().NotBeEmpty();
        });
    }
    ...
    public async Task DeveObterObjetosDeMesmoIdEmCachesDiferentesAsync()
    {
        await ExecutarComSemaforoAsync(async () =>
        {
            var cache1 = CriarDoisNiveisCache<string>(BuscarItemAsync, true, id: "1");
            var cache2 = CriarDoisNiveisCache<string>(BuscarItemAsync, true, id: "1");
    ...
    public async Task DeveAdicionarERemoverItemDoCacheAsync()
    {
        await ExecutarComSemaforoAsync(async () =>
        {
            ...
            var cache = CriarDoisNiveisCache<string>(BuscarItemAsync, true, id: cacheId);
            ...
            var cacheParaRemover = CriarDoisNiveisCacheParaRemover<string>(true, id: cacheId);
