...
public class AtualizacaoMarcaDomainValidator : DomainValidator
{
    private readonly IMarcaRepository _marcaRepository;

    public AtualizacaoMarcaDomainValidator(IMarcaRepository marcaRepository)
    {
        _marcaRepository = marcaRepository;
    }

    public bool Validate(MarcaId marcaId, string nome, string sigla)
    {
        var nomeNaoEhUnico = _marcaRepository.GetQueryable()
            .Where(m => m.Id != marcaId &&
                m.Nome.Trim().ToUpper().Equals(nome.Trim().ToUpper()) &&
