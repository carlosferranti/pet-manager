...
public class CriteriosInfoCupomInscricaoSpecificationTests
{
    private readonly ICampoRepository _campoRepository;
    private readonly List<InfoValidation> _validations = new();

    public CriteriosInfoCupomInscricaoSpecificationTests(DatabaseFixture databaseFixture)
    {
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task DeveRetornarValidacaoDeCupomDependendoDaObrigatoriedadeAsync(bool obrigatorio)
    {
        // Arrange
        var specification = await ObterSpecification(obrigatorio);

        // Act
        var resultado = specification.IsSatisfiedBy(InscricaoConstants.InscricaoVazia.Cupons);

        // Assert
        resultado.Should().Be(!obrigatorio);
        _validations.Exists(x => x.Atributo == CampoConstants.Cupom.Nome).Should().Be(obrigatorio);
    }

    private async Task<CriteriosInfoCupomInscricaoSpecification> ObterSpecification(bool obrigatorioFicha = true)
    {
